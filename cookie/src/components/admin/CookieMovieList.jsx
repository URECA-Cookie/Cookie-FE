...
const CookieMovieList = () => {
  ...
  const createIconAction = (type, movie) => {
    switch (type) {
      case "like":
        return () => handleLikeClick(movie.movieId);
      case "edit":
        return () => {
  ...
  const setCategories = (updatedCategories) => {
    setSelectedMovie((prevMovie) => ({
      ...prevMovie,
      ...
      if (isConfirmed) {
        try {
          const response = await axiosInstance.delete(
            `${serverBaseUrl}/api/admin/movies`,
            {
  ...
  // const handleSetCategories = (updatedCategories) => {
    ...
    <>
      <AddMovieContainer>
              ...
              <React.Fragment key={movie.movieId}>
                ...
                <IconContainer className="grid-item">
                  {Icon.map((icon, index) => (
                    <IconButton
                      key={index}
                      onClick={createIconAction(icon.type, movie)}
                    >
        ...
        {isModalOpen && selectedMovie && (
          <MovieInfoModal movieId={selectedMovieId[0]} onClose={closeModal} />
        )}
        <Pagination
